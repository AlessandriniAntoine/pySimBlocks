"""
bench_02_multi_feedback — Comparison
======================================
...
Simulink timing (t_mean_sl, t_std_sl) must be updated manually after running the
Simulink model on the target machine.
"""
...
if __name__ == "__main__":
    ...
    # ── Timing arrays (ms) ────────────────────────────────────────────────────
    times_ps = np.array([r[3] for r in results_ps]) * 1e3
    ...
    noise_sl  = sl[:, 1]
    output_sl = sl[:, 2]
    t_mean_sl = 253.5   # ms — measured separately, update if re-running on a different machine
    t_std_sl  = 12.5    # ms
    ...
    logger.info(f"{'Comparison':<30} {'Time':>18} {'Noise':>18} {'Output':>18}")
    logger.info("-" * 88)
    logger.info(
    ...
    ax.set_title("bench_02 — Signal comparison")
