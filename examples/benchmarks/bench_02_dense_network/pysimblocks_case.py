...
def test_pysimblocks_case():
    sim, _ = load_simulator_from_project(BASE_DIR / 'project.yaml')
    ...
    noise_data = sim.get_data(block="WhiteNoise", port="out").squeeze()
    output_data = sim.get_data(block="Integrator", port="out").squeeze()
